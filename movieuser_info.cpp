...
//displays movie info
void movie_info::output() {
	cout << "City:" << city;
	cout << "Movie name : " << movie_name;
	cout << "Number of shows : " << no_of_shows;
	cout << "Timings";
	for (int i = 0; i < no_of_shows; i++) {
...
void user_info::input(movie_info& mi){
	char ct[15],mv[30];
	time t;
	...
	cout << "Enter city name : ";
	cin >> ct;
	if (compare_city_name(ct, mi))	strcpy_s(city, ct);

	//input movie name
	cout << "Enter movie name : ";
	cin >> mv;
	if (compare_movie_name(mv, mi))	strcpy_s(movie_name, mv);

	//input movie timing
	cout << "Enter movie time (hh:mm) : ";
	t.gettime();
	if (compare_movie_time(t, mi))	movie_time = t;
}
